// Copyright 2015 Reborndb Org. All Rights Reserved.
// Licensed under the MIT (MIT-LICENSE.txt) license.

package rdb

import (
	"bytes"
	"encoding/hex"
	"math"
	"strconv"
	"strings"
	"testing"

	gocheck "gopkg.in/check.v1"
)

func TestT(t *testing.T) {
	gocheck.TestingT(t)
}

var _ = gocheck.Suite(&testRedisRdbSuite{})

type testRedisRdbSuite struct {
}

func (s *testRedisRdbSuite) hexStringToObject(c *gocheck.C, ss string) interface{} {
	p, err := hex.DecodeString(strings.NewReplacer("\t", "", "\r", "", "\n", "", " ", "").Replace(ss))
	c.Assert(err, gocheck.IsNil)

	o, err := DecodeDump(p)
	c.Assert(err, gocheck.IsNil)
	c.Assert(o, gocheck.NotNil)

	return o
}

/*
#!/bin/bash
for i in 1 255 256 65535 65536 2147483647 2147483648 4294967295 4294967296; do
	./redis-cli set string ${i}
	./redis-cli dump string
done
./redis-cli set string "hello world!!"
./redis-cli dump string
s=""
for ((i=0;i<1024;i++)); do
	s="01"$s
done
./redis-cli set string $s
./redis-cli dump string
*/
func (s *testRedisRdbSuite) TestDecodeString(c *gocheck.C) {
	docheck := func(hexs string, expect string) {
		val := s.hexStringToObject(c, hexs).(String)
		c.Assert([]byte(val), gocheck.DeepEquals, []byte(expect))
	}

	docheck("00c0010600b0958f3624542d6f", "1")
	docheck("00c1ff0006004a42131348a52fa4", "255")
	docheck("00c1000106009cb3bb1c58e36c78", "256")
	docheck("00c2ffff0000060047a5299686680606", "65535")
	docheck("00c200000100060056e7032772340449", "65536")
	docheck("00c2ffffff7f0600ba998d1e157b9132", "2147483647")
	docheck("000a323134373438333634380600715c4123b9484a7d", "2147483648")
	docheck("000a3432393439363732393506009a94b642c60c15f2", "4294967295")
	docheck("000a343239343936373239360600334ee148efd97ac5", "4294967296")

	docheck("000d68656c6c6f20776f726c64212106004aa70c88a8ad3601", "hello world!!")
	var b bytes.Buffer
	for i := 0; i < 1024; i++ {
		b.Write([]byte("01"))
	}
	docheck("00c31f480002303130e0ff01e0ff01e0ff01e0ff01e0ff01e0ff01e0ff01e0ba010130310600bcd6e486102c99c7", b.String())
}

/*
#!/bin/bash
for ((i=0;i<32;i++)); do
	./redis-cli rpush list $i
done
./redis-cli dump list
*/
func (s *testRedisRdbSuite) TestDecodeListZipmap(c *gocheck.C) {
	ss := `
		0a405e5e0000005a000000200000f102f202f302f402f502f602f702f802f902
		fa02fb02fc02fd02fe0d03fe0e03fe0f03fe1003fe1103fe1203fe1303fe1403
		fe1503fe1603fe1703fe1803fe1903fe1a03fe1b03fe1c03fe1d03fe1e03fe1f
		ff060052f7f617938b332a
	`
	val := s.hexStringToObject(c, ss).(List)
	c.Assert(len(val), gocheck.Equals, 32)

	for i := 0; i < len(val); i++ {
		c.Assert(string(val[i]), gocheck.Equals, strconv.Itoa(i))
	}
}

/*
#!/bin/bash
for ((i=0;i<32;i++)); do
	./redis-cli rpush list $i
done
./redis-cli dump list
*/
func (s *testRedisRdbSuite) TestDecodeList(c *gocheck.C) {
	ss := `
		0120c000c001c002c003c004c005c006c007c008c009c00ac00bc00cc00dc00e
		c00fc010c011c012c013c014c015c016c017c018c019c01ac01bc01cc01dc01e
		c01f0600e87781cbebc997f5
	`
	val := s.hexStringToObject(c, ss).(List)
	c.Assert(len(val), gocheck.Equals, 32)

	for i := 0; i < len(val); i++ {
		c.Assert(string(val[i]), gocheck.Equals, strconv.Itoa(i))
	}
}

/*
#!/bin/bash
for ((i=0;i<32;i++)); do
	./redis-cli sadd set $i
done
./redis-cli dump set
*/
func (s *testRedisRdbSuite) TestDecodeSet(c *gocheck.C) {
	ss := `
		0220c016c00dc01bc012c01ac004c014c002c017c01dc01cc013c019c01ec008
		c006c000c001c007c00fc009c01fc00ec003c00ac015c010c00bc018c011c00c
		c00506007bd0a89270890016
	`
	val := s.hexStringToObject(c, ss).(Set)
	c.Assert(len(val), gocheck.Equals, 32)

	set := make(map[string]bool)
	for _, mem := range val {
		set[string(mem)] = true
	}

	c.Assert(len(val), gocheck.Equals, len(set))

	for i := 0; i < 32; i++ {
		_, ok := set[strconv.Itoa(i)]
		c.Assert(ok, gocheck.Equals, true)
	}
}

/*
#!/bin/bash
for ((i=0;i<32;i++)); do
	let j="$i*$i"
	./redis-cli hset hash $i $j
done
./redis-cli dump hash
*/
func (s *testRedisRdbSuite) TestDecodeHash(c *gocheck.C) {
	ss := `
		
	`
	val := s.hexStringToObject(c, ss).(Hash)
	c.Assert(len(val), gocheck.Equals, 32)

	hash := make(map[string]string)
	for _, ent := range val {
		hash[string(ent.Field)] = string(ent.Value)
	}

	c.Assert(len(val), gocheck.Equals, len(hash))

	for i := 0; i < 32; i++ {
		s := strconv.Itoa(i)
		c.Assert(hash[s], gocheck.Equals, strconv.Itoa(i*i))
	}
}

/*
#!/bin/bash
for ((i=0;i<32;i++)); do
	./redis-cli zadd zset -$i $i
done
./redis-cli dump zset
*/
func (s *testRedisRdbSuite) TestDecodeZSet(c *gocheck.C) {
	ss := `
		
		16
	`
	val := s.hexStringToObject(c, ss).(ZSet)
	c.Assert(len(val), gocheck.Equals, 32)

	zset := make(map[string]float64)
	for _, ent := range val {
		zset[string(ent.Member)] = ent.Score
	}

	c.Assert(len(val), gocheck.Equals, len(zset))

	for i := 0; i < 32; i++ {
		s := strconv.Itoa(i)
		score, ok := zset[s]
		c.Assert(ok, gocheck.Equals, true)
		c.Assert(math.Abs(score+float64(i)) < 1e-10, gocheck.Equals, true)
	}
}
